var $ = (function(d) {
  var slice = [].slice, k,jsonpID = (new Date).getTime(),
  ...
  function ajax(method, url, success, params){
      var re = /callback=\?/;
      if (re.test(url)) { // JSONP
        var jsonpString = 'jsonp' + ++jsonpID;
        window[jsonpString] = function(j) { success(j) }
        var script = d.createElement('script');
        $(script).attr({ src: url.replace(re, 'callback=' + jsonpString), type: 'text/javascript' });
        $('head').append(script);
      } else {
    ...
    $.get = function(url, success, params){ ajax('GET', url + (params || ''), success); };
    $.post = function(url, success, params){ ajax('POST', url, success, params); };
    $.getJSON = function(url, success, params){
      $.get(url, function(json){ success(typeof json == 'string' ? JSON.parse(json) : json) }, params);
    };  return $;
})(document);